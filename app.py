...
def load_my_predictions():
    ...
    try:
        df = pd.read_csv(PREDICTIONS_FILE, sep=";", encoding="utf-8-sig", dtype=str)
    except Exception:
        return pd.DataFrame(columns=PREDICTION_COLUMNS)
...
def save_my_prediction(record):
    df = load_my_predictions()
    df = pd.concat([df, pd.DataFrame([record])], ignore_index=True)
    df.to_csv(PREDICTIONS_FILE, sep=";", index=False, encoding="utf-8-sig")
...
def delete_my_prediction(prediction_id):
    ...
    if df.empty:
        return
    df = df[df["id"].astype(str) != str(prediction_id)]
    df.to_csv(PREDICTIONS_FILE, sep=";", index=False, encoding="utf-8-sig")
...
with right:
    if run:
        ...
        section("Moja predikcia")

        with st.form("my_prediction_form"):
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                my_p1_aces = st.number_input(f"{short_name(player1)} esá", min_value=0.0, step=0.5, value=0.0)
            with c2:
                my_p1_df = st.number_input(f"{short_name(player1)} DF", min_value=0.0, step=0.5, value=0.0)
            with c3:
                my_p2_aces = st.number_input(f"{short_name(player2)} esá", min_value=0.0, step=0.5, value=0.0)
            with c4:
                my_p2_df = st.number_input(f"{short_name(player2)} DF", min_value=0.0, step=0.5, value=0.0)

            my_note = st.text_input("Poznámka", value="")
            save_prediction = st.form_submit_button("💾 Uložiť zápas")
        ...
        if save_prediction:
            ...
            st.success("Zápas bol uložený do zoznamu.")

    else:
        st.info("Vyber hráčov a klikni na Vypočítať.")

    section("Uložené predikcie")

    saved_predictions = load_my_predictions()

    if saved_predictions.empty:
        st.info("Zatiaľ nemáš uložené žiadne zápasy.")
    else:
        csv_data = saved_predictions.to_csv(sep=";", index=False).encode("utf-8-sig")
        st.download_button(
            "⬇️ Stiahnuť CSV",
            data=csv_data,
        ...
        for _, row in saved_predictions.iloc[::-1].iterrows():
            title = (
                f"{row.get('created_at', '')} | "
                f"{row.get('player1', '')} vs {row.get('player2', '')} | "
                f"{row.get('tour', '')} {row.get('surface', '')} | "
                f"{row.get('og', '')} gemov"
            )

            with st.expander(title):
                st.write(
                    f"**{row.get('player1', '')}**: esá {row.get('p1_aces', '')}, "
                    f"DF {row.get('p1_df', '')}"
                )
                st.write(
                    f"**{row.get('player2', '')}**: esá {row.get('p2_aces', '')}, "
                    f"DF {row.get('p2_df', '')}"
                )

                if row.get("note", ""):
                    st.write(f"Poznámka: {row.get('note', '')}")

                if st.button("🗑️ Vymazať", key=f"delete_{row.get('id', '')}"):
                    delete_my_prediction(row.get("id", ""))
                    st.rerun()
