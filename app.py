...
def level_filter(df, tour, level):
    if not include_qual:
        df = df[~df["Round"].isin(["Q1", "Q2", "Q3"])
        & ~df["Tournament"].astype(str).str.contains(EXCLUDED_TOURNAMENTS, case=False, na=False)
    ]
...
with right:
    if run:
        ...
        # Debug export zápasov pre kontrolu filtrov
        debug_player = player1

        debug_matches = df[df["Player"] == debug_player].copy()
        ...
        debug_cols = [c for c in debug_cols if c in debug_matches.columns]
        debug_matches = debug_matches[debug_cols]

        debug_matches = debug_matches.sort_values("Date", ascending=False)

        st.download_button(
            label=f"Stiahnuť zápasy: {debug_player}",
            data=debug_matches.to_csv(index=False, sep=";", decimal=",", encoding="utf-8-sig"),
            file_name=f"{debug_player.replace(' ', '_')}_{surface}_{level}_{year_from}_{year_to}.csv",
            mime="text/csv",
        )     

        top100_mask = df["IsTop100"] == True
        
        if player1 == "Yannick Hanfmann":
            st.write("Počet zápasov v databáze:", len(df[df["Player"] == player1]))

            st.dataframe(
            df[df["Player"] == player1][
                ["Date", "Tournament", "Surface", "Level", "Opponent"]
